...
# DEV (single-user cluster): keep PAT, pop SP OAuth → authenticates as the user.
# PROD (GIS-GAE cluster): pop PAT, keep SP OAuth → authenticates as the app SP.
...
# catching analytical queries like "what zip code in CO has the most deliveries".
...
# ---------------------------------------------------------------------------
# Generic spatial containment: polygons x points -> which points are inside?
# The operation is the same regardless of what produced the polygons
# (service areas, weather alerts, boundaries, custom uploads) or what the
# points represent (facilities, collection boxes, delivery points, etc.).
# ---------------------------------------------------------------------------
...
def _geojson_to_wkt(geom: Dict[str, Any]) -> str:
    ...
    raise ValueError(f"Unsupported geometry type for containment: {gtype}")
...
def _bbox_from_geojson_features(features: List[Dict[str, Any]]) -> Optional[tuple]:
...
def _point_in_polygon(px, py, polygon_coords):
    """Ray-casting point-in-polygon test. polygon_coords is a list of [lon, lat] pairs."""
...
def _point_in_geojson(px, py, geom):
    """Check if point (lon, lat) is inside a GeoJSON Polygon or MultiPolygon."""
    gtype = geom.get("type", "")
    coords = geom.get("coordinates", [])
    if gtype == "Polygon":
        # Check outer ring (index 0), ignore holes for simplicity
        return _point_in_polygon(px, py, coords[0]) if coords else False
    if gtype == "MultiPolygon":
        return any(_point_in_polygon(px, py, poly[0]) for poly in coords if poly)
    return False


def _build_containment_code(polygon_wkts: List[str], polygon_props: List[Dict[str, Any]], point_sql: str) -> str:
    """Generate cluster code that fetches points as JSON (no ST_Contains).

    The actual containment is done Python-side in _run_containment() for speed.
    This just retrieves candidate points from the bbox-filtered SQL.
    """
    return _build_code(
        _SPARK_SETUP,
        "import json",
        ...
        "try:",
        "    rows = spark.sql(point_sql).collect()",
        "    points = []",
        "    for r in rows:",
        "        points.append({k: r[k] for k in r.asDict()})",
